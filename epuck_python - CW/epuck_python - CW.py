...
class Controller:
    ...
    def sense_compute_and_actuate(self):
        ...
        self.velocity_left = self.max_speed if (output[0] > self.max_speed) else output[0]
        self.velocity_right = self.max_speed if (output[1] > self.max_speed) else output[1]
        
        # self.velocity_left = 0 if (self.velocity_left > -0.1 and self.velocity_left < 0.1) else self.velocity_left
        # self.velocity_right = 0 if (self.velocity_right > -0.1 and self.velocity_right < 0.1) else self.velocity_right
    ...
    def calculate_fitness(self):
        ...
        l_vel = self.left_motor.getVelocity()
        r_vel = self.right_motor.getVelocity()
        
        # print(self.inputs[1:], l_vel, r_vel)
        
        forwardFitness = 0 # ?
        
        if (l_vel > 0) and (r_vel > 0):
            forwardFitness += 10 * (l_vel + r_vel)
        elif (l_vel < 0) and (r_vel < 0):
            # forwardFitness += -10 + 10 * (l_vel + r_vel)
            self.crashed = True
        ...
        # print(self.inputs[1:])
        for v in range(len(self.inputs[1:])):
            if self.inputs[1:][v] > 0.5:
                # avoidCollisionFitness -= 10 * self.inputs[1:][v]
                self.crashed = True
                if not self.demo: return
            else:
                avoidCollisionFitness -= 10 * self.inputs[1:][v]
        ...
        spinningFitness = 0 # ?
        val = 0.4
        if (l_vel > val and r_vel < -val) or (l_vel < -val and r_vel > val):
            self.crashed = True
            if not self.demo: return
        elif ((l_vel < val and l_vel >= 0) and (r_vel > -val and r_vel <= 0)) or ((l_vel > -val and l_vel <= 0) and (r_vel < val  and r_vel >= 0)):
            spinningFitness += 2 * (np.abs(l_vel) + np.abs(r_vel))
        elif (l_vel == 0 and r_vel != 0) or (l_vel != 0 and r_vel == 0):
            spinningFitness += 10 * (np.abs(l_vel) + np.abs(r_vel))
 
        
        ### Define the fitness function of this iteration which should be a combination of the previous functions
        # print(forwardFitness, avoidCollisionFitness, spinningFitness)
    ...
    def handle_receiver(self):
        if self.receiver.getQueueLength() > 0:
            while(self.receiver.getQueueLength() > 0):
                ...
                if self.receivedData.split()[-1] == 'or':
                    self.found_square = False
                    self.crashed = False
                    self.demo = False
                    self.receivedData = self.receivedData[0:-2]
                elif self.receivedData.split()[-1] == 'dr':
                    print("demo")
                    self.found_square = False
                    self.crashed = False
                    self.demo = True
                    self.receivedData = self.receivedData[0:-2]
                self.receivedData = self.receivedData[1:-2]
    ...
    def run_robot(self):        
        ...
        while self.robot.step(self.time_step) != -1:
            ...
            if (center-min_gs)/(max_gs-min_gs) < 0.4:
